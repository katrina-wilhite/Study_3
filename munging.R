...
munge <- function(sex) {
  ...
  relevant_sex_data <- df %>%
    select(hicid, model_used, Age, Remoteness, Indigenous, SEP)
  ...
  wave_6_relevant_data <- lsac_wave6 %>%
    ...
    rename("adv" = "fcnfsad2d", "dis" = "fcnfsda2d", "resources"= "fcnfser2d", "education_occupation" = "fcnfseo2d", "mother_home" = "fmoth", "father_home" = "ffath", "mental_health" = "fsc13a1i", "mother_race" = "zf09fm", "father_race" = "zf09ff")
  wave_8_relevant_data <- lsac_wave8 %>%
    select(hicid, hapsoc:hasdqtb) %>% 
    rename("prosocial" = "hapsoc", "hyperactivity" = "hahypr", "emotional" = "haemot", "peer" = "hapeer", "conduct" = "hacondb", "total" = "hasdqtb")
