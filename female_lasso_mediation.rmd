...
exposure_variable <- df_females %>% 
  select(SEP) 
exposure_variable <- exposure_variable$SEP 
```
...
for(i in 1:ncol(df_outcomes)) {
  ...
  dat.filter <- assign(paste0("dat.filter.", outcome), regmed.prefilter(x, med, y, k = 10)) 
  x1 <- dat.filter$x
  y1 <- dat.filter$y
...
plot.regmed.edges(edges.any.hapsoc.single)
