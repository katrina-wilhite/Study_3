...
model_selection_table$outcomes <- model_selection_list_names
model_selection_table <- model_selection_table %>% 
  select(outcomes, everything())
```

```{r include=FALSE}
model_selection_table$outcomes <- stringr::str_remove(model_selection_table$outcomes, "female_")
model_selection_table$outcomes <- stringr::str_remove(model_selection_table$outcomes, "male_")
model_selection_table$outcomes <- stringr::str_remove(model_selection_table$outcomes, "_single")

model_selection_table <- model_selection_table %>% 
  mutate(outcomes = str_replace(outcomes, "hapsoc", "Prosocial Behaviour")) %>% 
  mutate(outcomes = str_replace(outcomes, "hahypr", "Hyperactivity-Inattentiveness")) %>% 
  mutate(outcomes = str_replace(outcomes, "haemot", "Emotional Problems ")) %>% 
  mutate(outcomes = str_replace(outcomes, "hapeer", "Peer Problems")) %>% 
  mutate(outcomes = str_replace(outcomes, "hacondb", "Conduct Problems")) %>% 
  mutate(outcomes = str_replace(outcomes, "hasdqtb", "Total Score")) %>% 
  
model_selection_table <- dplyr::rename(model_selection_table, 'Socio-Emotional Outcomes'=outcomes) 
  
```

```{r echo=FALSE}
model_selection_table %>% 
  kbl(caption = "Table 3. Lasso Model Selection") %>% 
  kable_classic(html_font = "Arial") %>% 
  pack_rows("Males", 1,12) %>% 
  pack_rows("Females", 13, 24) %>% 
  pack_rows("Single Movement Behaviours", 1,6) %>% 
  pack_rows("Combinations of Movement Behaviours", 7, 12) %>%
  pack_rows("Single Movement Behaviours", 13,18) %>% 
  pack_rows("Combinations of Movement Behaviours", 19, 24) %>% 
